...
func initTable(db *sql.DB) {
	statement, err := db.Prepare(
		`CREATE TABLE IF NOT EXISTS quedou ("date" TEXT, 
"hour" TEXT,
"room_id" TEXT,
"room_name" TEXT,
"store_name" TEXT, 
...
func insertRoom(db *sql.DB, date string, hour string, store interface{}, room interface{}) {
	...
	// 插入数据
	statement, err := db.Prepare(
		`INSERT INTO quedou (date, hour, room_id, room_name, store_name, store_id, store_address, price, status) 
	...
	_, err = statement.Exec(date, hour, roomID, roomName, storeName, storeID, storeAddress, price, status)
	if err != nil {
		fmt.Println(err)
	}
}

func StartWorkOnQuedou(db *sql.DB, date string, hour string, token string) {
	initTable(db)
	...
	for _, store := range stores.([]interface{}) {
		...
		wg.Add(1)
		go func() {
			handleStore(db, date, hour, curStore, token)
			wg.Done()
		}()
	}
	wg.Wait()
...
func handleStore(db *sql.DB, date string, hour string, store interface{}, token string) {
	rooms := GetRooms(store.(map[string]interface{}), token)
	for _, room := range rooms.([]interface{}) {
		insertRoom(db, date, hour, store, room)
	}
}
